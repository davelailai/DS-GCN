...
@LOSSES.register_module()
class SimilarityLoss(BaseWeightedLoss):
    """Binary Cross Entropy Loss with logits.
    ...
    def _forward(self, x, label, **kwargs):
        """Forward function.
        ...
        for i in range(N):
            for j in range(N):
               label_similarity[i,j]= label[i]==label[j] 
...
@LOSSES.register_module()
class SimLoss(BaseWeightedLoss):
    """Binary Cross Entropy Loss with logits.
    ...
    def _forward(self, x, label, **kwargs):
        ...
        loss_same = label_similarity * torch.pow(res1, 2)
        loss_diff = (1-label_similarity) * torch.pow(torch.clamp(self.margin - res1, min=0.0), 2)
