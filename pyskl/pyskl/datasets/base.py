...
from ..core import mean_average_precision, mean_class_accuracy, top_k_accuracy, confusion_matrix, TSNEmap
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def evaluate(self,
        ...
        allowed_metrics = ['top_k_accuracy', 'mean_class_accuracy', 'mean_average_precision','confusion_matrix','TSNEmap','graph']
        ...
        for metric in metrics:
            ...
            if metric == 'mean_class_accuracy':
                mean_acc, _ = mean_class_accuracy(results, gt_labels)
            ...
            if metric == 'confusion_matrix':
                _, cf_mat = mean_class_accuracy(results, gt_labels)
            ...
            if metric == 'TSNEmap':
                ...
                result = TSNEmap(np.array(results), gt_labels)
                eval_results['TSNEmap'] = result
                eval_results['labels'] = gt_labels
                continue
            ...
            if metric == 'graph':
                ...
                for i in range(max(label)):
                    ...
                    index_graph = results[index,:,:,:].mean(1).mean(0)
                    result.append(index_graph)
                eval_results['graph'] = result
                continue
    ...
    def prepare_train_frames(self, idx):
        ...
        if self.memcached and 'key' in results:
            ...
            pack = data[key]
            results['keypoint'] = pack['keypoint']
        # if self.memcached and 'key' in results:
        #     from pymemcache.client.base import Client
        #     from pymemcache import serde

        #     if self.cli is None:
        #         self.cli = Client(self.mc_cfg, serde=serde.pickle_serde, connect_timeout=20.0,timeout=20.0)
        #         # self.cli = Client((self.mc_cfg, 11211))
        #     key = results.pop('key')
        #     try:
