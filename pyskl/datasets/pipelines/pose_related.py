...
from .causal import *
from .plot_confusion_metric import *
from .Neural_GC_master.models.clstm import cLSTM, train_model_ista
...
#         # else:
#         #     keypoint = keypoint.squeeze(0).transpose(1,2,0)

        
#         keypoint = results['keypoint'].mean(0).transpose(2,0,1)
#         keypoint = torch.from_numpy(keypoint)
#         C, T, V = keypoint.shape
#         clstm = cLSTM(keypoint.shape[-1], hidden=100)
#         train_loss_list = train_model_ista(
#             clstm, keypoint, context=10, lam=10.0, lam_ridge=1e-2, lr=1e-3, max_iter=20000,
#             check_every=50)
#         GC_est = clstm.GC().cpu().data.numpy()
#         causal_matrix = pTE(keypoint, model_order=3, to_norm=True)
#         results['causal'] = causal_matrix
#         # labels = range(25)
#         # plot_confusion_matrix(causal_matrix,labels)
#         return results
@PIPELINES.register_module()
class GaussAug:
    """calculate Causal Metrix """
    def __init__(self, thr=0.5, ratio=1e-2):
        self.thr = thr
        self.ratio = ratio
    def __call__(self, results):
        # index = random()
        if random()>self.thr:
            r = self.ratio
            keypoint = results['keypoint']
            mask_convariance_maxtix = [[r, 0, 0],
                                    [0, r, 0],
                                    [0, 0, r]]
            center = np.array([0, 0, 0])
            N, T, V, C = keypoint.shape
            num = keypoint.reshape(-1,C).shape[0]
            aug = np.random.multivariate_normal(center, mask_convariance_maxtix, num)
            aug = aug.reshape(N,T,V,C)
            keypoint = keypoint + aug
            results['keyoint'] = keypoint 
...
class Causalmetrix:
    ...
    def __call__(self, results):
        ...
        # else:
        #     keypoint = keypoint.squeeze(0).transpose(1,2,0)
        # keypoint = results['keypoint'].mean(0).transpose(1,2,0)
        causal_matrix = results['causal']
        # causal_matrix = pTE(keypoint, model_order=3, to_norm=True)
        thr = np.percentile(causal_matrix, self.thr)
        causal_matrix[causal_matrix<thr]=0

        # causal_matrix = np.zeros(25,25)
        results['causal'] = causal_matrix
        # labels = range(25)
        # plot_confusion_matrix(causal_matrix,labels)
        return results
...
class FormatGCNInput:
    ...
    def __call__(self, results):
        ...
        if keypoint.shape[0] < self.num_person:
            ...
            #     causal = causal[np.newaxis, :].repeat(self.num_person,0)
            #     results['causal'] = np.ascontiguousarray(causal)
